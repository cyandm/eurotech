(() => {
  ...
  function slideTo(index, speed, runCallbacks, internal, initial) {
    ...
    if (swiper3.animating && params.preventInteractionOnTransition || !enabled && !internal && !initial || swiper3.destroyed) {
      return false;
    }
    ...
    function handle(event2) {
      ...
      if (!swiper3.params.freeMode || !swiper3.params.freeMode.enabled) {
        ...
        if (!ignoreWheelEvents) {
          ...
          if (!ignoreWheelEvents)
            emit("scroll", e);
          if (swiper3.params.autoplay && swiper3.params.autoplayDisableOnInteraction)
  ...
  function Controller(_ref) {
    ...
    on("beforeInit", () => {
      ...
      (typeof swiper3.params.controller.control === "string" || swiper3.params.controller.control instanceof HTMLElement)) {
        const controlElement = document.querySelector(swiper3.params.controller.control);
  ...
  function A11y(_ref) {
    ...
    function onEnterOrSpaceKey(e) {
      ...
      if (swiper3.navigation && swiper3.navigation.nextEl && targetEl === swiper3.navigation.nextEl) {
      ...
      if (swiper3.navigation && swiper3.navigation.prevEl && targetEl === swiper3.navigation.prevEl) {
    ...
    extendParams({
      hashNavigation: {
        ...
        getSlideIndex(_s, hash) {
          if (swiper3.virtual && swiper3.params.virtual.enabled) {
            const slideWithHash = swiper3.slides.filter((slideEl) => slideEl.getAttribute("data-hash") === hash)[0];
            if (!slideWithHash)
              return 0;
  ...
  function Thumb(_ref) {
    ...
    function update2(initial) {
      ...
      if (swiper3.realIndex !== thumbsSwiper.realIndex || useOffset) {
        ...
        let direction;
        if (thumbsSwiper.params.loop) {
          const newThumbsSlide = thumbsSwiper.slides.filter((slideEl) => slideEl.getAttribute("data-swiper-slide-index") === "".concat(swiper3.realIndex))[0];
  ...
  function effectVirtualTransitionEnd(_ref) {
    ...
    const getSlide = (el) => {
      if (!el.parentElement) {
        const slide2 = swiper3.slides.filter((slideEl) => slideEl.shadowRoot && slideEl.shadowRoot === el.parentNode)[0];
        return slide2;
      }
    ...
    const setTranslate2 = () => {
      ...
      for (let i = 0; i < slides.length; i += 1) {
        ...
        if (progress <= 1 && progress > -1) {
          ...
          if (rtl)
            wrapperRotate = -slideIndex * 90 - progress * 90;
          if (swiper3.browser && swiper3.browser.need3dFix && Math.abs(wrapperRotate) / 90 % 2 === 1) {
            wrapperRotate += 1e-3;
          }
        }
      ...
      wrapperEl.style.transform = "translate3d(0px,0,".concat(zFactor, "px) rotateX(").concat(swiper3.isHorizontal() ? 0 : wrapperRotate, "deg) rotateY(").concat(swiper3.isHorizontal() ? -wrapperRotate : 0, "deg)");
    ...
    const setTranslate2 = () => {
      ...
      for (let i = 0; i < slides.length; i += 1) {
        ...
        r.forEach((value, index) => {
          ...
          if (swiper3.browser && swiper3.browser.need3dFix && Math.abs(val) / 90 % 2 === 1) {
            val += 1e-3;
          }
          r[index] = val;
        });
